...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    #can move distinct before joins, but pluck has to go at the end.
    # ordered_items_names = Item.distinct.joins(:orders).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # alternatively can break select statement into multiple select statements
        # data = User
        # .joins(:order_items)
        # .group("users.name, orders.id")
