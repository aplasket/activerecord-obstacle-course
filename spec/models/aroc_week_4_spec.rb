...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales =  Order.where.not(user: @user_2).sum(:amount).to_i
    # -----------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    # orders = Order.joins(:orderitems).where(order_items: {item_id: @item_4.id})
    # binding.pry
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    # orders = Order.joins(:order_items).where("orders.user_id = #{@user_2.id}").where("order_items.item_id = #{@item_4.id}")
    #alternatively:
    orders = Order.joins(:items).where(user: @user_2).where(items: {id: @item_4.id})
    # -----------------------------------------------------------
