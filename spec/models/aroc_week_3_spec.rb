...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      users = User.joins(:order_items).distinct.where( order_items: { item_id: @item_8.id }).order(:name).pluck(:name)
      #alternative:
      # users = User.joins(:order_items).distinct.where( "order_items.item_id = #{@item_8.id }").order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
        items_for_user_3_third_order = Order.where(user: @user_3).offset(2).first.items.pluck(:name)
    # ------------------------------------------------------------
